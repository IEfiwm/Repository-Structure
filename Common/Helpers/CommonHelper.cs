...
namespace Common.Helpers
{
    public static class CommonHelper
    {
        ...
        public static string SendWebRequest(string url, string data, params string[][] headers)
        {
            using (var client = new WebClient())
            {
                client.Headers[HttpRequestHeader.ContentType] = "application/json";
        ...
        public static Task<string> SendWebRequestAsync(string url, string data)
        {
            using (var client = new WebClient())
            {
                client.Headers[HttpRequestHeader.ContentType] = "application/json";
                ...
                return client.UploadStringTaskAsync(url, data);
            }
        }

        public static Task<string> SendWebRequestAsync(string url, string data, string username, string password)
        {
            using (var client = new WebClient())
            {
                client.Headers[HttpRequestHeader.ContentType] = "application/json";
        ...
        public static void SendWebRequest(string url, string data, string username, string password)
        {
            using (var client = new WebClient())
            {
                client.Headers[HttpRequestHeader.ContentType] = "application/json";
        ...
        public static async Task<string> CrawlAsync(string url, Encoding encoding)
        {
            using (var webClient = new WebClient())
            {
                webClient.Headers.Add("user-agent",
                    "Mozilla/5.0 (Windows NT 6.2; WOW64) AppleWebKit/537.11 (KHTML, like Gecko) Chrome/23.0.1271.97 Safari/537.11 AlexaToolbar/alxg-3.1");
        ...
        public static async Task<string> GetOriginUrl(string url)
        {
            ...
            using (webResponse = (HttpWebResponse)await webRequest.GetResponseAsync())
            {
                ...
                return webResponse.ResponseUri.Host + webResponse.Headers["Location"];
            }
        }

        public static string ReadWebRequest(string url)
        {
            using (var client = new WebClient())
            {
                client.Headers[HttpRequestHeader.ContentType] = "application/json";
        ...
        public static Task<string> ReadWebRequestAsync(string url, string username, string password)
        {
            using (var client = new WebClient())
            {
                client.Headers[HttpRequestHeader.ContentType] = "application/json";
        ...
        public static Task<string> ReadWebRequestAsync(string url, string method, string data, Dictionary<string, string> headers)
        {
            using (var client = new WebClient())
            {
                client.Encoding = Encoding.UTF8;
        ...
        public static Task<string> SendWebRequestAsync(string url, string method, string data)
        {
            using (var client = new WebClient())
            {
                client.Encoding = Encoding.UTF8;
        ...
        public static async Task<TModel> ReadWebRequestAsync<TModel>(string url)
        {
            using (var client = new WebClient())
            {
                client.Encoding = Encoding.UTF8;
        ...
        public static string SerializeJsonAndEncrypt<T>(this T parameter, string key = "")
        {
            if (parameter == null)
            {
            ...
            {
                key = MainConstants.DefaultKey;
            }

            return Encrypt(parameter.SerializeJson(), key);
        }

        public static T DeserializeJsonAndDecrypt<T>(this string parameter, string key = "")
        {
            ...
            return Deserialize<T>(Decrypt(parameter, key));
        }
        ...
        public static string GenerateJwtToken(string id, double days = 7)
        {
            var secret = new ConfigurationBuilder()
                .AddJsonFile("appsettings.json")
                .Build()
                .GetSection("AppConfigs")["Secret"];
